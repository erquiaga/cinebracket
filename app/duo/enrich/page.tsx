...
export default function DuoEnrichPage() {
  ...
  useEffect(() => {
    ...
    if (!raw) { router.replace('/duo'); return; }
    ...
    enrichMovies(movies, (d, t, name) => { setDone(d); setTotal(t); setCurrentName(name); })
      .then(async (enriched) => {
        ...
        await saveWatchlist('duo', enriched);
        setFinished(true);
      })
  ...
  return (
    <main className={styles.main}>
      <div className={styles.headerArea}>
        <Button type='text' icon={<ArrowLeftOutlined />} onClick={() => router.push('/duo')} disabled={!finished && !error} className={styles.backBtn}>
          Back
        </Button>
        ...
        <motion.div initial={{ opacity: 0, y: -16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45, ease: 'easeOut' as const }}>
          <Title level={2} className={styles.pageTitle} style={{ fontFamily: 'var(--font-folio), sans-serif', fontWeight: 'normal', fontSize: 'clamp(1.8rem, 4vw, 2.8rem)', letterSpacing: '0.04em' }}>
            <span className={styles.duoAccent}>GATHERING DATA</span>
          </Title>
      ...
      <motion.div className={styles.contentArea} initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.1, ease: 'easeOut' as const }}>
        {error ? (
          ...
          <EnrichmentProgress done={done} total={total} currentName={currentName} accentColor='#00e054' />
        )}
      </motion.div>
      ...
      <motion.div className={styles.contentArea} initial={{ opacity: 0 }} animate={{ opacity: finished ? 1 : 0.3 }} transition={{ duration: 0.3 }}>
        <Button
          disabled={!finished}
          onClick={() => router.push('/duo/filter')}
          className={`${styles.continueBtn} ${finished ? styles.continueBtnGreenActive : styles.continueBtnInactive}`}
        >
          Continue to Filters →
        </Button>
      </motion.div>
